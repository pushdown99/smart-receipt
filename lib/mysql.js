...
module.exports = {
  ...
  findReceiptUidRno: function(record) {
    console.log(record);
    return this.query("SELECT rno, uid, cname, rcn, fname, items FROM receipts WHERE uid = ? AND rno = ?", record)[0];
  },
  findReceiptRcnRno: function(record) {
    console.log(record);
    return this.query("SELECT rno, uid, cname, rcn, fname, items FROM receipts WHERE rcn = ? AND rno = ?", record)[0];
  },
  ...
  getBooks:  function(record) {
    return this.query("SELECT * FROM books ORDER BY registered DESC LIMIT 500", record);
  },
