...
module.exports = function (data) {
  ...
  for(var index = 0; index < data.length; index++) {
    ...
    // basic control command
      ...
      case 0x1B: // ESC
        ...
        switch (data[index+1]) {
          ...
          case 0x61: // ESC a n, Position alignment
            index += 2; break;
          case 0x7b: // ESC { n, Turn on/off upside-down printing mode.
            index += 2; break;
      ...
      case 0x1D: // GS
        ...
        switch (data[index+1]) {
          ...
          case 0x6B: // GS k m …, Print bar code 
            console.log("-> m n:",  data[index+2], data[index+3]);
          ...
          case 0x22: // Print character on the curve
            ...
            index += 1; break;
          default:
            console.log("Unknown GS/POS command", util.format("GS %s", String.fromCharCode(data[index+1])));
            index += 1;
        }
        break;
      ...
      default:
        //if(data[index] != 0) {
        if(data[index] == 0x22) {
          console.log ("Illegal: ", String.fromCharCode(data[index]), data[index].toString(16));
        }
        else if(data[index] >= 0x20) {
          //console.log (String.fromCharCode(data[index]), data[index].toString(16));
          //console.log(String.fromCharCode(data[index]), data[index], data[index].toString(16));
          buf.push(data[index]);
        }
        else {
          console.log ("Illegal: ", String.fromCharCode(data[index]), data[index].toString(16));
  ...
  //var str = Buffer.from(buf);
  var enc = detectCharacterEncoding(Buffer.from(buf));
  console.log('Encode=>', enc);
  //console.log('Encode:', iconv.encode(str,'euc-kr'));
  //if (enc["encoding"] == "ISO-8859-1") {
  //  console.log("IOS-8859-1 == ",  enc);
  //  str = iconv.encode(str,'euc-kr');
  //}
  //return iconv.decode(str, 'euc-kr').toUpperCase();
